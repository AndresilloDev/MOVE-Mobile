    ...
    const handleDelete = async () => {
        try {
            ...
            if (response.status === 200) {
                ...
                getError("Error al eliminar el usuario. Por favor, inténtelo de nuevo más tarde.");
            }
        } catch (error) {
            getError("Error al eliminar el usuario. Por favor, inténtelo de nuevo más tarde.");
        }
    ...
    return (
        <View className="flex-1 bg-gray-100">
            ...
            <View className="m-4 ml-6 flex-row items-center">
                ...
                <Text className="text-xl">Eliminar Usuario</Text>
            </View>

            {/* Contenido central */}
            <View className="flex-1 items-center justify-center px-4 gap-10">
                <Text className="text-2xl font-bold mb-4 text-center">Eliminar Usuario</Text>
                <Text>
                    ¿Estás seguro de que deseas eliminar al usuario <Text className="font-bold">{user.user}</Text>?
                </Text>
