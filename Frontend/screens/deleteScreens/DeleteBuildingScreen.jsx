...
const DeleteBuildingScreen = () => {
    ...
    const handleDelete = async () => {
        try {
            const response = await deleteBuilding(building._id);
    ...
    return (
        <View className="flex-1 bg-gray-100">
            ...
            <View className="m-4 ml-6 flex-row items-center">
                ...
                <Text className="text-xl">Eliminar Edificio</Text>
            </View>

            {/* Contenido central */}
            <View className="flex-1 items-center justify-center px-4 gap-10">
                <Text className="text-2xl font-bold mb-4 text-center">Eliminar Edificio</Text>
                <Text>
                    ¿Estás seguro de que deseas eliminar el edificio <Text className="font-bold">{building.name}</Text>?
                </Text>
